...
// Bump when the compiled math changes so older files on disk are rebuilt, not reused.
// 2: CO scheduled minutes are counted per day (lib/co-scheduled-minutes.js); older
//    files summed about a week of schedule into one day.
export const COMPILED_REPORT_VERSION = 2;
...
export async function buildCompiledReport(
  ...
  if (useCache && fs.existsSync(outPath)) {
    try {
      ...
      const version = normalized?.report?.meta?.compiledVersion;
      if (normalized && version !== COMPILED_REPORT_VERSION) {
        console.warn(
          `[report-builder] Ignoring ${outPath}: compiled version ${version ?? "none"}, need ${COMPILED_REPORT_VERSION}`
        );
        progress("cache-stale", `Cached compiled report for client ${normalizedId} is out of date; rebuilding`);
      } else if (normalized) {
        progress(
          "cache-hit",
  ...
  const report = {
    meta: {
      ...
      compiledVersion: COMPILED_REPORT_VERSION,
    },
