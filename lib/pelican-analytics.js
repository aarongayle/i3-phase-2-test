...
export function calculatePelicanAnalytics(thermostats, buildings = [], dateRange) {
  ...
  for (const t of thermostats || []) {
    ...
    if (!byThermostat.has(serialNo)) {
      byThermostat.set(serialNo, {
        ...
        occupancyMinutes: 0,
        runtimeMinutes: 0,
        coolRuntimeMinutes: 0,
        heatRuntimeMinutes: 0,
        unoccupiedDayRuntimeMinutes: 0,
        fanMinutes: 0,
        occupiedHeatSum: 0,
        occupiedHeatCount: 0,
        unoccupiedHeatSum: 0,
    ...
    agg.coolRuntimeMinutes += toMinutes(t.coolRuntime);
    agg.heatRuntimeMinutes += toMinutes(t.heatRuntime);
    // Runtime on a day the thermostat never went occupied (weekends, holidays, overrides).
    if (occupiedMinutes === 0) agg.unoccupiedDayRuntimeMinutes += runtimeMinutes;
  ...
  const thermostatStats = Array.from(byThermostat.values()).map((agg) => {
    ...
    return {
      ...
      coolRuntimeMinutes: agg.coolRuntimeMinutes,
      heatRuntimeMinutes: agg.heatRuntimeMinutes,
      unoccupiedDayRuntimeMinutes: agg.unoccupiedDayRuntimeMinutes,
