...
export async function getClients() {
  const response = await coFetch(`${uri}/campus-optimizer/clients`);
...
export async function getUtilityTypes() {
  const response = await coFetch(`${uri}/types/utility-types`);
  const data = await response.json();
  return data;
}

export async function expectedEnergyUse(client, options = {}) {
  const { onProgress, reportWindowDays, allReportDates } = options;
  ...
  progress("expected-start", "Loading actual energy for shape alignment", {
    client,
  });
  const actual = await actualEnergyUse(client, { onProgress });

  // 2) Get all report dates, normalized and sorted asc (YYYY-MM-DD)
  progress("expected-dates", "Fetching report dates for expected energy", {
    client,
  });
  const reportDatesRaw = await getReportDates(client);
  const allReportDays = Array.from(
    new Set(
      (allReportDates?.length
        ? allReportDates
        : (reportDatesRaw || [])
            .map((d) => d?.report_date)
            .filter(Boolean)
  ...
  let reportDays = allReportDays;
  if (reportWindowDays != null && allReportDays.length > reportWindowDays) {
    reportDays = allReportDays.slice(allReportDays.length - reportWindowDays);
    progress("expected-dates-window", "Bounded schedule-details window", {
      totalReportDays: allReportDays.length,
      windowReportDays: reportDays.length,
      windowDays: reportWindowDays,
    });
  }

  progress("expected-dates", "Report dates normalized", {
    totalReportDays: allReportDays.length,
    fetchingScheduleDetailsFor: reportDays.length,
  });

  if (reportDays.length === 0) {
    // No reports; return zeros aligned to actuals
    progress("expected-empty", "No report days; filling zeros");
  ...
  progress("expected-schedules", "Fetching schedule details", {
  ...
  const schedulesByDate = await Promise.all(
    reportDays.map((d, i) =>
      coFetch(`${uri}/schedule-details?client=${client}&date=${d}`, {
        ...
        .then((rows) => {
          completedSchedules++;
          // Emit periodic progress updates so long streams stay alive
          if (
            completedSchedules === 1 ||
            completedSchedules === reportDays.length ||
            completedSchedules % 5 === 0
          ) {
            progress("expected-schedules", "querying schedules", {
              completed: completedSchedules,
              total: reportDays.length,
              date: d,
              message: `querying schedules ${completedSchedules}/${reportDays.length}`,
            });
          }
          return rows;
        })
  ...
  progress("expected-schedules-done", "All schedule details fetched", {
    totalReportDays: reportDays.length,
  });
  ...
  const nearestReportPool =
    reportWindowDays != null ? reportDays : allReportDays;
  const dateToReport = new Map();
  allDays.forEach((day) => {
    let closest = nearestReportPool[0];
    let minDiff = Math.abs(new Date(day) - new Date(nearestReportPool[0]));
    for (let j = 1; j < nearestReportPool.length; j++) {
      const diff = Math.abs(new Date(day) - new Date(nearestReportPool[j]));
      if (diff < minDiff) {
        minDiff = diff;
        closest = nearestReportPool[j];
      }
    }
  ...
  progress("expected-done", "Expected energy ready", {
    meters: expected.length,
  });
  return expected;
}

export async function actualEnergyUse(client, options = {}) {
  const { onProgress } = options;
  const progress = (stage, message, extra) =>
    onProgress?.({ stage, message, ...extra });

  progress("actual-start", "Fetching meters and interval data", { client });
  const meters = await coFetch(`${uri}/project/meters?client=${client}`);
  const metersData = await meters.json();
  progress("actual-meters", "Meters fetched", { count: metersData.length });

  const response = await coFetch(`${uri}/trends/interval?client=${client}`);
  const data = await response.json();
  progress("actual-intervals", "Interval data fetched");
  ...
  progress("actual-done", "Actual energy ready", {
    meters: result.length,
  });
  return result;
}
