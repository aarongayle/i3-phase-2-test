...
export function buildPelicanRollups(summaries, window = {}) {
  ...
  // Older cached days can have blank names/groups (e.g. an early backfill); use each unit's latest non-blank values.
  const identity = new Map();
  for (const s of rows) {
    const id = identity.get(s.serialNo) || { name: "", nameDate: "", groupName: "", groupDate: "" };
    if (s.name && s.date >= id.nameDate) Object.assign(id, { name: s.name, nameDate: s.date });
    if (s.groupName && s.date >= id.groupDate) Object.assign(id, { groupName: s.groupName, groupDate: s.date });
    identity.set(s.serialNo, id);
  }
  const units = new Map();
  ...
  for (const s of rows) {
    const { name, groupName } = identity.get(s.serialNo);
    const group = groupName || "Unknown";
    const m = s.metrics || null;

    if (group === HIDDEN_GROUP) {
      // Hidden devices are usually plant sensors (chilled / hot water loops).
      if (m?.loop) {
        const loop = loops.get(s.serialNo) || { name, serialNo: s.serialNo, days: [] };
    ...
    const u =
      ...
      { serialNo: s.serialNo, name, groupName: group, days: 0, coolH: 0, heatH: 0, occH: 0, unoccDayRunH: 0,
