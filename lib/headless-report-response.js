...
export function buildHeadlessReportResponse({
  ...
  if (splitImages) {
    ...
    if (noImages && !result?.analytics) {
      throw new Error("Report images were not generated");
    }
    if (noImages) {
      // Analytics still go out so follow-up Q&A works without chart images.
      payload.status = "partial";
      payload.renderError = result?.renderError ?? "Report images were not generated";
    }
