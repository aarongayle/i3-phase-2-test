...
function buildCoDailyRuntimeMap(devices) {
  ...
  for (const device of devices || []) {
    for (const point of device.runtimeDaily || []) {
...
function buildPelicanThermostatLookup(thermostats) {
  ...
  for (const stat of thermostats || []) {
    ...
    const serial = normalizeSerial(stat.serialNo);
    if (serial) bySerial.set(serial, stat);
  }
  return { byName, bySerial };
}

/** CO device → Pelican thermostat by serial (preferred), then by name. */
function findPelicanMatch(lookup, device) {
  const serial = normalizeSerial(device.pelicanSerialNo);
  return (
    (serial && lookup.bySerial.get(serial)) ||
    lookup.byName.get(normalizeName(device.name)) ||
    null
  );
}
...
export function buildReportAnalytics(fullData, options = {}) {
  try {
    ...
    const energyExpected = Array.isArray(energy.expected) ? energy.expected : [];
    const energyActual = Array.isArray(energy.actual) ? energy.actual : [];
    const meterCatalog = Array.isArray(options.meterCatalog)
      ? options.meterCatalog
      : [];
    const meterCatalogById = new Map(
      meterCatalog
        .filter((meter) => meter?.meterId != null)
        .map((meter) => [String(meter.meterId), meter])
    );
    ...
    const latestEnergyDate = energyLabels.length
      ...
      : null;
    const energyComparisonLabels = comparisonWindowLabels(
      energyLabels,
      devices,
      dailyExpectedMap
    );
    ...
    return {
      ...
      meterCatalog,
      reports: [slice],
      // Every unit, per building, per month, comfort/humidity and plant loops (see lib/pelican-rollups.js).
      pelicanRollups: fullData?.pelican?.rollups ?? null,
    };
  } catch (err) {
...
function buildMeters(ctx) {
  ...
  const allMeters = Array.from(meterIdSet).map((id) => {
    ...
    const catalogMeter =
      id != null ? ctx.meterCatalogById.get(String(id)) : null;
    return {
      meterName: meterNameFor(id, ctx.energyExpected, ctx.energyActual),
      meterId: id != null ? String(id) : null,
      meterNumber: catalogMeter?.meterNumber ?? null,
      esiId: catalogMeter?.esiId ?? null,
      accountNumber: catalogMeter?.accountNumber ?? null,
      utilityType: catalogMeter?.utilityType ?? null,
      delta: round2(delta),
      deltaUnit: "kWh",
...
/**
 * Dates where expected and actual energy can be compared like for like.
 * Actual meter data covers the full history, but CO's expected forecast only
 * exists for the bounded report window, so summing every date inflates the
 * "miss" by months of actuals with no forecast behind them.
 */
function comparisonWindowLabels(energyLabels, devices, dailyExpectedMap) {
  const coDates = Array.from(buildCoDailyRuntimeMap(devices).keys()).sort();
  let start = coDates[0];
  let end = coDates[coDates.length - 1];
  if (!start) {
    start = energyLabels.find((date) => (dailyExpectedMap.get(date) || 0) > 0);
    end = energyLabels[energyLabels.length - 1];
  }
  if (!start) return [];
  return energyLabels.filter((date) => date >= start && date <= end);
}

function buildEnergySummary(ctx) {
  let totalExpected = 0;
  let totalActual = 0;
  const labels = ctx.energyComparisonLabels ?? ctx.energyLabels;
  ...
  return {
    comparedFrom: labels[0],
    comparedTo: labels[labels.length - 1],
    comparedDays: labels.length,
...
function buildDevices(ctx) {
  ...
  const ranked = aggregationService
    ...
    .map((device) => {
      ...
      const pelicanMatch = findPelicanMatch(pelicanLookup, device);
      const flags = [];
      ...
      if (
        ...
        flags.push("runtime_over_occupied");
      }
      ...
      return {
        ...
        coDeviceId: device.coDeviceId ?? null,
        groupName: pelicanMatch?.groupName ?? null,
        // "pelican": compressor runtime ÷ occupied time. "co_schedule": CO scheduled
        // minutes ÷ 24 h (no Pelican link), which is schedule length, not runtime.
        runtimeSource: pelicanMatch ? "pelican" : "co_schedule",
        runtimePercent: round2(effectiveRuntimePercent),
        scheduledMinutesPerDay: round2(runtimeMinutes),
        coolingMinutes: pelicanMatch ? round2(pelicanMatch.coolRuntimeMinutes) : null,
        heatingMinutes: pelicanMatch ? round2(pelicanMatch.heatRuntimeMinutes) : null,
        occupiedMinutes: pelicanMatch ? round2(pelicanMatch.occupancyMinutes) : null,
        unoccupiedDayRuntimeMinutes: pelicanMatch
          ? round2(pelicanMatch.unoccupiedDayRuntimeMinutes)
          : null,
        latentRuntimeMinutes: null,
        fanMinutes: pelicanMatch ? round2(pelicanMatch.fanMinutes) : null,
        flags: flags.length ? flags : undefined,
        _linked: Boolean(pelicanMatch),
        _sortRuntime: pelicanMatch ? effectiveRuntimePercent : runtimeMinutes,
      };
    });

  // Linked devices first, by actual runtime ÷ occupied; unlinked ones after, by schedule.
  ranked.sort(
    (a, b) =>
      Number(b._linked) - Number(a._linked) ||
      (b._sortRuntime || 0) - (a._sortRuntime || 0)
  );

  const allDevices = ranked.map((device, index) => {
    const { _sortRuntime, _linked, ...rest } = device;
...
function buildScheduleCompliance(pelicanAnalytics, devices) {
  ...
  for (const date of combinedDates) {
    ...
    // A day with no CO data is unknown, not zero scheduled minutes.
    if (!pelicanDay || !coDailyMap.has(date)) continue;
    const coMinutes = coDailyMap.get(date);
  ...
  return {
    ...
    // Runtime above scheduled minutes can't distinguish overrides from units that
    // never satisfy at setback, so this is not inferred.
    manualOverrideSuspected: null,
    comparedDays,
...
function buildDiagnostics(ctx) {
  ...
  const overOccupied = ctx.devices.filter((d) =>
    d.flags?.includes("runtime_over_occupied")
  );
  if (overOccupied.length > 0) {
    findings.push({
      code: "runtime_over_occupied",
      severity: "warning",
      title: "Units running longer than their occupied time",
      detail:
        "Compressor runtime exceeds occupied time by more than 10%; these units may not be satisfying at setback.",
      affectedDevices: overOccupied.slice(0, 5).map((d) => d.deviceName),
      evidence: { deviceCount: overOccupied.length },
    });
  }
  ...
  if ((ctx.scheduleCompliance?.unscheduledRuntimeMinutes || 0) > 120) {
    findings.push({
      ...
      severity: "info",
      title: "Pelican runtime above CO scheduled minutes",
      detail:
        "Site-wide daily compressor minutes exceeded CO scheduled minutes on some days. This compares two different measures and is not evidence of overrides on its own.",
      evidence: {
        unscheduledRuntimeMinutes:
          ctx.scheduleCompliance.unscheduledRuntimeMinutes,
        comparedDays: ctx.scheduleCompliance.comparedDays,
      },
    });
  }
  ...
  // A day with no CO forecast (weekends, holidays) can't be "over expected".
  if (worstMeter && (worstMeter.delta ?? 0) > 0 && (worstMeter.expected ?? 0) > 0) {
    findings.push({
