...
export class DataAggregationService {
  ...
  async aggregateDeviceMetrics(devices, dates, clientId) {
    ...
    const sortedDates = Array.from(
      new Set(
        (dates || [])
          .map((d) => this._normalizeReportDate(d?.report_date))
          .filter(Boolean)
      )
    ...
    return Array.from(deviceAggregates.values()).map((agg) => {
      ...
      return {
        ...
        coDeviceId: agg.id ?? null,
        pelicanSerialNo: agg.pelicanSerialNo,
  ...
  _initializeDeviceAggregates(devices, units) {
    ...
    devices.forEach((device) => {
      aggregates.set(device.Id, {
        ...
        pelicanSerialNo: device.PelicanSerialNo ?? null,
  ...
  _normalizeReportDate(dateValue) {
